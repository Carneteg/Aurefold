...
```mermaid
flowchart TD
    ALAINE[Mother Alaine\nHead of Whitehart\nNever heard] -->|appoints / governs| TOMAS[Brother Tomas\nThe Tester]
    ALAINE -->|institutional authority| WILDA[Sister Wilda\nLaw and rite]
    ALAINE -->|uses / trusts| OSRIC[Brother Osric\nAmbitious institutional mind]
    ALAINE -->|depends on| ANSEL[Steward Ansel\nFood, order, redistribution]

    TOMAS -->|tests| SELA[Sela\n19, goatherd, Called]
    TOMAS -->|tested five years earlier| FEN[Fen\n24, Unwritten, niche-tender]
    TOMAS -->|judged false| COL[Col\nMerta's son, fate unresolved]

    MERTA[Merta\n52, Col's mother] -->|mother of| COL
    WILDA -->|let him go| COL

    SELA -->|friendship / trust / unspoken injury| FEN
    FEN -->|sells useful truths, hides the cost| PERRIN[Perrin\nBlackcrest Jubilee Herald\nNo POV]
    PERRIN -->|writes / reframes| SELA
    PERRIN -->|offers corrected entry| MERTA
    MERTA -->|refuses| PERRIN

    HARL[Harl\nValley worker and guide] -->|mentor / practical kinship| SELA
    HARL -->|guardian / provider| TAM[Tam\nPromised a horse]
    HARL -->|teaches work| BREM[Brem\nSela's younger brother]
    SELA -->|sister of| BREM

    IVET[Ivet\n16, novice believer] -->|interprets everything as confirmation| SELA
    BRYN[Bryn Corr\nBeds, bread, bookings] -->|profits from / is harmed by| SELA

    ORLA[Orla\nStonebear oath-witness] -->|rules the boundary| WHITEHART[House Whitehart]
    NELLA[Nella\n46, Duskport trader] -->|feeds the valley through ledgers and carts| WHITEHART
    ROAN[Roan\n29, Stormrider-linked traveler] -->|brings contradictory Garron reports| WHITEHART
    OTTAR[Ottar\n38, carries Kavan's unexplained scars] -->|travels with| NELLA

    WENNA[Wenna\nNamed gate victim] -->|human cost of| GATE[The Gate]
    WICK[Wick\nSaint in one account, butcher in another] --> GATE
    VASS[Ordin Vass\nRescuer who fits no clean version] --> GATE
```
