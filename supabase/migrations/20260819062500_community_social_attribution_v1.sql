...
begin
  ...
  window_counts as (
    select jsonb_build_object(
      ...
      'campaign_entries', count(distinct visitor_id) filter (where event_name = 'community_entry')::integer,
  ...
  acquisition as (
    select coalesce(jsonb_agg(jsonb_build_object(
      'source', source_key,
      'entries', entries,
      'ledger_voters', ledger_voters,
      'returning_participants', returning_participants,
      'patreon_intent', patreon_intent
    ) order by entries desc, source_key), '[]'::jsonb) as data
    from (
      select
        e.context_key as source_key,
        count(distinct e.visitor_id)::integer as entries,
        count(distinct e.visitor_id) filter (where v.ledger_questions_answered >= 1)::integer as ledger_voters,
        count(distinct e.visitor_id) filter (where v.ledger_questions_answered >= 2)::integer as returning_participants,
        count(distinct e.visitor_id) filter (where v.patreon_intent)::integer as patreon_intent
      from public.community_funnel_events e
      left join aurefold_private.community_funnel_visitors v on v.visitor_id = e.visitor_id
      where e.event_name = 'community_entry'
        and e.context_key is not null
  ...
  daily as (
    ...
    left join (
      select created_at::date as day,
             count(distinct visitor_id) filter (where event_name='community_entry')::integer as entry,
  ...
  select jsonb_build_object(
    ...
    'acquisition_sources', acquisition.data,
    ...
    'definitions', jsonb_build_object(
      'campaign_entry', 'browser arrival through an Aurefold campaign link carrying an explicit src marker',
  ...
  into v_result
  from funnel, overlap, stage_counts, window_counts, patreon, acquisition, houses, ledger, sworn_houses, daily;
