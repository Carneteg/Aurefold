...
begin
  ...
  select jsonb_build_object(
    ...
    'definitions', jsonb_build_object(
      ...
      'sworn_reader', 'created or saved a Banner reader profile',
      'patreon_intent', 'clicked a Patreon CTA; not confirmed membership or revenue'
    )
  )
  into v_result
  from funnel, stage_counts, window_counts, patreon, houses, ledger, sworn_houses, daily;
...
comment on function public.author_community_dashboard(integer) is
'Author/admin-only aggregated community manager dashboard. Returns no raw visitor identifiers and treats Patreon clicks as intent only.';
